...
describe('FileSystemSkillProvider', () => {
  ...
  it('can omit the shared agents skill root while retaining project and dsh roots', async () => {
    const home = await tempDir('skill-without-agents-home')
    const project = await tempDir('skill-without-agents-project')
    await mkdir(join(project, '.git'), { recursive: true })
    await writeSkill(join(project, '.agents/skills'), 'project-skill', 'Project skill')
    await writeSkill(join(home, '.dsh/skills'), 'dsh-skill', 'DSH skill')
    await writeSkill(join(home, '.agents/skills'), 'shared-skill', 'Shared skill')

    const ctx = await setupLocal(home, { includeAgentsHome: false })
    const skills = await ctx.skills.list({ cwd: join(project, 'src') })

    expect(skills.map(skill => skill.name)).toEqual(['dsh-skill', 'project-skill'])
  })
