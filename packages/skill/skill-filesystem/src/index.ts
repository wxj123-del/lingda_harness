...
export const Config: Schema<Config> = z.object({
  ...
  includeAgentsHome: z.boolean().default(true),
  dshHome: z.string(),
  agentsHome: z.string(),
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  private readonly includeAgentsHome: boolean
  ...
  constructor(
    ...
    this.includeAgentsHome = config.includeAgentsHome ?? true
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        ...
        ...this.includeAgentsHome
          ? [{ path: join(this.agentsHome, 'skills'), source: 'user-agents' as const, rank: USER_AGENTS_RANK }]
          : [],
      )
    }
