...
describe('PluginInventorySettingsTab', () => {
  ...
  it('filters the inventory by semantic plugin category', async () => {
    const view = await renderReady()
    const tools = view.container.querySelector('[data-plugin-category="tools"]')
    expect(tools?.textContent).toContain(en.categoryTools)
    expect(view.container.querySelector('[data-plugin-category="rag"]')).toBeNull()

    fireEvent.click(tools as HTMLElement)
    expect(view.container.querySelector('[data-plugin-scope="preset"] [data-plugin-entry="bash"]')).toBeTruthy()
    expect(view.container.querySelector('[data-plugin-scope="preset"] [data-plugin-entry="fs"]')).toBeTruthy()
    expect(view.container.querySelector('[data-plugin-scope="preset"] [data-plugin-entry="codex"]')).toBeNull()
    expect(view.container.querySelector('[data-plugin-count]')?.getAttribute('data-plugin-count')).toBe('2')
    expect(view.container.querySelector('[data-preset-plugin-count]')?.getAttribute('data-preset-plugin-count')).toBe('3')
    expect(view.container.querySelector('[data-plugin-scope="global"] [data-plugin-entry="bash-host"]')).toBeTruthy()

    fireEvent.change(screen.getByRole('searchbox', { name: en.search }), { target: { value: 'no-such-tool' } })
    expect(view.container.querySelector('[data-plugin-category="tools"]')?.getAttribute('aria-selected')).toBe('true')
    fireEvent.change(screen.getByRole('searchbox', { name: en.search }), { target: { value: '' } })
  ...
  it('groups image generation and voice model plugins under model-related plugins', async () => {
    const view = await renderReady({
      entries: [
        { entryId: 'image-gen', moduleName: '@fixture/image-generation', enabled: true, fiberPhase: null },
        { entryId: 'tts', moduleName: '@fixture/tts', enabled: true, fiberPhase: null },
      ],
    } as unknown as Snapshot)

    const modelsCategory = view.container.querySelector('[data-plugin-category="models"]')
    expect(modelsCategory?.textContent).toContain(en.categoryModels)
