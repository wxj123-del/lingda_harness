...
describe('Conversation inject API', () => {
  ...
  it('projects the dynamic View registration ledger', async () => {
    ...
    const removeReplacement = b.slots.register(
      { name: 'conversation.view', id: 'trajectory', priority: -10, order: 5, label: 'Token trace' },
      (() => null) as never,
    )
    await vi.waitFor(() => {
      expect(source.getSnapshot()).toEqual([{ id: 'trajectory', label: 'Token trace' }])
    })
    removeReplacement()
    await vi.waitFor(() => {
      expect(source.getSnapshot()).toEqual([{ id: 'trajectory', label: 'Trajectory' }])
