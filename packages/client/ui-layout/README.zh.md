...
本包提供 Web GUI 的三栏 AppFrame、左右栏宽度与 `ctx.layout` 呈现控制，也提供用户创建独立 Skill、工作流、工具和知识库插件的编辑界面。知识库支持手动录入资料或上传文本资料，每个保存项目都通过自己的设置记录隔离。右栏先让步以保护中栏空间，全屏由占用方呈现，框架保留宽屏底层轨道。主题呈现器负责配色、别名 token、正文字号与 document 元数据；布局状态在刷新后重置。
...
编辑导航会把 Skill、工作流、工具和知识库保存到 `ui-authoring` 设置命名空间。每个项目都有名称、描述、启用状态和详情内容；Skill 与工作流使用有序步骤，知识库保存相互独立的资料。知识库编辑器支持手动添加文本资料，也支持一次上传多个本地文本文件（`txt`、Markdown、JSON、CSV 和 YAML），在浏览器中读取内容后保存到当前知识库。
...
Host 注册两个内置创建 Skill 和已启用的用户 Skill。名称与描述通过 `tool-skill` 进入技能目录，完整指令按需加载或通过显式 `/name` 调用加载。用户描述包含展示标题，方便自然语言点名匹配。内置 Skill 生成可复用的编辑器内容，不声称已保存或执行。用户工作流和工具返回指令；知识库工具对已保存资料进行关键词检索。
