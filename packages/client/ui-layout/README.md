...
This package provides the Web GUI's three-column AppFrame, edge-column widths, and `ctx.layout` presentation control. It also provides the user authoring surface for independent Skill, workflow, tool, and knowledge-base plugins. Knowledge bases can contain manually entered documents or uploaded text materials, and each saved item remains isolated by its own settings record. The right column concedes space before the center; its occupant renders fullscreen while the frame retains the wide-screen track underneath. The theme presenter owns color scheme, alias tokens, content font size, and document metadata. Layout state resets on reload.
...
The authoring navigation stores Skills, workflows, tools, and knowledge bases in the `ui-authoring` settings namespace. Each item has a name, description, enabled state, and detail content. Skills and workflows use ordered steps; knowledge bases keep separate documents. The knowledge editor accepts manual text documents and multiple local text files (`txt`, Markdown, JSON, CSV, and YAML), reading file contents in the browser before saving them with the selected knowledge base.
