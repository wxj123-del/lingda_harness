...
import { registerAuthoringSurface } from './authoring/register.tsx'
...
export { AUTHORING_SETTINGS_NAMESPACE } from '../authoring-settings.ts'
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  ...
  interface SlotMap {
    ...
    /** Narrow product navigation rail rendered inside the sidebar column. */
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => {
    ...
    retainMainPanels()
    registerAuthoringSurface(ctx, instance)
    return () => {
      layout.dispose()
      disposePanels()
      disposeRegistration()
