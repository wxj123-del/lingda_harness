...
/** Read an optional provider modality extension without guessing absent data. */
function inputModalities(...candidates: readonly unknown[]): readonly ModelModality[] | undefined {
  for (const candidate of candidates) {
    if (!Array.isArray(candidate)) continue
    const values = candidate.filter((value): value is ModelModality => value === 'text' || value === 'image')
    if (values.length !== candidate.length && candidate.length > 0) continue
