...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  beforeAll(async () => {
    ...
    const sharedSkill = join(scaffold.workspaceCwd, '.agents-home', 'skills', 'unrelated-global-skill')
    await mkdir(sharedSkill, { recursive: true })
    await writeFile(join(sharedSkill, 'SKILL.md'), [
      '---',
      'name: unrelated-global-skill',
      'description: Unrelated global instructions that must not enter the default prompt.',
      '---',
      '',
      'Unrelated global skill body.',
      '',
    ].join('\n'))
  ...
  it('ends the system prompt with the source checkout, Web surface, and session cwd', async () => {
    ...
    expect(system).not.toContain('unrelated-global-skill')
