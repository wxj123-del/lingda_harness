...
describe('web e2e: agent-preset selection', () => {
  ...
  it('names every preset and what it is for', async () => {
    ...
    expect(snapshot).toContain('Budget mode')
    await page.keyboard.press('Escape')
  })

  it('selects Budget mode and keeps it after a reload', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-agent-preset-budget'))
    await page.getByRole('button', { name: 'Standard mode', exact: true }).click()
    await page.getByRole('menuitem', { name: /^Budget mode/ }).click()
    await expect.poll(() => livePreset(scaffold), { timeout: 15_000 }).toBe('diaosi')
    await page.reload({ waitUntil: 'load' })
    await page.getByRole('button', { name: 'Budget mode', exact: true }).waitFor({ timeout: 15_000 })
    await expect.poll(() => livePreset(scaffold), { timeout: 15_000 }).toBe('diaosi')
    try {
      await scaffold.ctx.settings.update('locale', { preference: 'zh' })
      await page.getByRole('button', { name: '屌丝模式', exact: true }).click()
      const snapshot = await captureStableAria(page, '[role="menu"]', scaffold.workspaceCwd)
      expect(snapshot).toContain('屌丝模式')
      await compareOrRefreshGolden(join(SNAPSHOT_DIR, 'menu-zh.expected.md'), snapshot, MODE)
      await page.screenshot({ path: join(tmpdir(), 'dsh-diaosi-desktop.png') })
      await page.keyboard.press('Escape')
      await page.getByRole('button', { name: '收起侧边栏', exact: true }).click()
      await page.setViewportSize({ width: 390, height: 844 })
      await expect.poll(async () => (await page.locator('[class*="heroWorkspaceRow"]').boundingBox())?.x ?? Infinity)
        .toBeLessThan(120)
      await page.getByRole('button', { name: '屌丝模式', exact: true }).click()
      await page.screenshot({ path: join(tmpdir(), 'dsh-diaosi-mobile.png') })
      expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
    } finally {
      await page.keyboard.press('Escape')
      await page.setViewportSize({ width: 1680, height: 1000 })
      await scaffold.ctx.settings.update('locale', { preference: 'en' })
      await page.getByRole('button', { name: 'Open sidebar', exact: true }).click()
    }
    await page.getByRole('button', { name: 'Budget mode', exact: true }).click()
