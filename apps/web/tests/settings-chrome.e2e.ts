...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    // The inventory is an independent product page, outside the Settings modal.
    ...
    const presetSwitcher = pluginPage.getByRole('button', { name: '选择要查看的 Agent 预设' })
    ...
    await pluginPage.getByRole('button', { name: /^全局/ }).click()
    const pluginRow = pluginPage.locator(PLUGIN_ROW_SELECTOR)
    await pluginRow.waitFor({ timeout: 10_000 })
    const expectedPluginCount = [...scaffold.ctx.loader.entries()]
      .filter(entry => !entry.options.group)
      .length
    const pluginSearch = pluginPage.getByRole('searchbox', { name: '搜索插件' })
    expect(await pluginSearch.count()).toBe(1)
    const toolsCategory = pluginPage.locator('[data-plugin-category="tools"]')
    await toolsCategory.click()
    expect(await toolsCategory.getAttribute('aria-selected')).toBe('true')
    await pluginPage.locator('[data-plugin-category="all"]').click()
    ...
    expect(await pluginPage.locator('[data-plugin-scope="global"] [data-plugin-entry]').count())
      .toBe(expectedPluginCount)
    expect(await pluginPage.locator('[data-plugin-count]').getAttribute('data-plugin-count'))
      .toBe(String(expectedPluginCount))
    expect(await pluginPage.getByRole('button', { name: '插件', exact: true }).getAttribute('aria-current')).toBe('page')
    ...
    for (const [entryId, status] of instanceRows) {
      const row = pluginPage.locator(`[data-plugin-scope="preset"] [data-plugin-entry="${entryId}"]`)
    ...
    await pluginPage.getByRole('button', {
      name: 'tool-subagent, tool-subagent-claude-code, 已停用',
      exact: true,
    }).click()
    expect(await pluginPage.locator('[data-plugin-entry="tool-subagent-claude-code"] button')
      .getAttribute('aria-expanded')).toBe('true')
    await pluginSearch.fill('')
    expect(await page.getByRole('dialog', { name: '设置' }).count()).toBe(0)
    // Close path 1: Escape.
    await trigger.click()
    expect(await page.getByRole('dialog', { name: '设置' }).getByRole('button', { name: '插件', exact: true }).count()).toBe(0)
    await page.keyboard.press('Escape')
  ...
  it('opens the authoring navigation and persists workflow steps', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-authoring-navigation'))
    const navigation = page.getByRole('navigation', { name: '产品导航' })
    await navigation.getByRole('button', { name: '工作流', exact: true }).click()
    await page.locator('h1').getByText('工作流', { exact: true }).waitFor({ timeout: 10_000 })
    await page.getByRole('button', { name: '新建', exact: true }).click()
    await page.getByLabel('名称', { exact: true }).fill('周报整理')
    await page.getByLabel('描述', { exact: true }).fill('整理本周工作并输出摘要')
    await page.getByPlaceholder('步骤...').fill('读取本周会议纪要')
    await page.getByPlaceholder('步骤...').press('Enter')
    await page.getByPlaceholder('步骤...').fill('生成摘要并保存')
    await page.getByRole('button', { name: '保存', exact: true }).click()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 10_000 })
      .toMatch(/ui-authoring:/)
    await expect.poll(() => {
      const tools = scaffold.ctx.get('tools') as { schemas: () => readonly { description: string }[] }
    ...
    await navigation.getByRole('button', { name: '工作流', exact: true }).click()
    await expect.poll(() => page.getByRole('button', { name: /周报整理/ }).count(), { timeout: 10_000 }).toBe(1)
    await navigation.getByRole('button', { name: '知识库', exact: true }).click()
    await page.locator('h1').getByText('知识库', { exact: true }).waitFor({ timeout: 10_000 })
    await page.getByRole('button', { name: '新建', exact: true }).click()
    await page.getByLabel('名称', { exact: true }).fill('产品资料')
    await page.getByLabel('描述', { exact: true }).fill('产品说明与常见问题')
    await page.getByRole('button', { name: '添加资料', exact: true }).click()
    await page.getByLabel('资料名称 1', { exact: true }).fill('产品说明')
    await page.getByLabel('资料内容 1', { exact: true }).fill('这是产品的核心说明。')
    await page.getByLabel('上传资料', { exact: true }).setInputFiles({
      name: '产品FAQ.md',
      mimeType: 'text/markdown',
      buffer: Buffer.from('# 常见问题\n\n这是上传的资料。'),
    })
    await page.getByLabel('资料名称 2', { exact: true }).waitFor({ timeout: 10_000 })
    await page.getByRole('button', { name: '保存', exact: true }).click()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 10_000 })
      .toMatch(/产品资料[\s\S]*产品FAQ\.md/)
    await expect.poll(() => {
      const tools = scaffold.ctx.get('tools') as { schemas: () => readonly { description: string }[] }
      return tools.schemas().some(tool => tool.description === '产品说明与常见问题')
    }, { timeout: 10_000 }).toBe(true)
    await page.getByRole('button', { name: '插件', exact: true }).click()
    const pluginPage = page
    await pluginPage.getByRole('heading', { name: '插件', exact: true }).waitFor({ timeout: 10_000 })
  ...
  it('keeps product navigation usable when conversation browsing is collapsed and on narrow screens', async () => {
    const navigation = page.getByRole('navigation', { name: '产品导航' })
    await navigation.getByRole('button', { name: '对话', exact: true }).click()
    await page.getByRole('button', { name: '收起侧边栏', exact: true }).click()
    await expect.poll(() => page.getByRole('button', { name: '新建会话', exact: true }).first().isVisible()).toBe(false)
    for (const label of ['对话', '知识库', '工作流', 'Skill', '工具', '插件']) {
      const button = navigation.getByRole('button', { name: label, exact: true })
      await button.click({ trial: true })
    }
    await navigation.getByRole('button', { name: '工作流', exact: true }).click()
    expect(await page.getByRole('button', { name: '新建会话', exact: true }).first().isVisible()).toBe(false)
    // Editing an existing item must retain typed changes and input focus.
    await page.getByRole('button', { name: /周报整理/ }).click()
    await page.getByLabel('描述', { exact: true }).fill('整理周报并保存修改')
    await page.getByLabel('步骤 1', { exact: true }).pressSequentially('，核对日期')
    expect(await page.getByLabel('步骤 1', { exact: true }).inputValue()).toContain('核对日期')
    await page.getByRole('button', { name: '保存', exact: true }).click()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8')).toContain('整理周报并保存修改')
    await navigation.getByRole('button', { name: '插件', exact: true }).click()
    await page.getByRole('searchbox', { name: '搜索插件' }).waitFor()
    await page.screenshot({ path: join(tmpdir(), 'dsh-plugins-desktop.png') })
    await page.setViewportSize({ width: 390, height: 844 })
    await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
    const catalog = page.locator('[data-plugins-page]')
    expect(await catalog.evaluate(element => element.scrollWidth <= element.clientWidth)).toBe(true)
    await page.getByRole('tab', { name: '插件配置', exact: true }).click()
    await page.getByRole('button', { name: '展开设置: 终端' }).click()
    expect(await page.getByLabel('命令超时（毫秒）').isVisible()).toBe(true)
    expect(await page.getByRole('dialog', { name: '设置' }).count()).toBe(0)
    await page.getByRole('tab', { name: '插件列表', exact: true }).click()
    await page.screenshot({ path: join(tmpdir(), 'dsh-plugins-mobile.png') })
    await navigation.getByRole('button', { name: '工作流', exact: true }).click()
    await page.getByRole('button', { name: /周报整理/ }).click()
    await page.getByLabel('名称', { exact: true }).scrollIntoViewIfNeeded()
    expect(await page.getByLabel('名称', { exact: true }).isVisible()).toBe(true)
    await page.screenshot({ path: join(tmpdir(), 'dsh-workflow-mobile.png') })
    await page.setViewportSize({ width: 1680, height: 1000 })
    await navigation.getByRole('button', { name: '对话', exact: true }).click()
    await page.getByRole('button', { name: '打开侧边栏', exact: true }).click()
    await expect.poll(() => page.getByRole('button', { name: '新建会话', exact: true }).first().isVisible()).toBe(true)
    await expect.poll(() => page.locator('[class*="sidebarCol"]').evaluate(element => Math.round(element.getBoundingClientRect().width))).toBe(280)
    await page.screenshot({ path: join(tmpdir(), 'dsh-conversation-desktop.png') })
    expect(tripwire.pageErrors).toEqual([])
  }, 60_000)
  ...
  it('opens an English browser in English without any stored preference', async () => {
    ...
    try {
      ...
      // The independent plugin page resolves shipped preset names through the
      // en dictionaries instead of echoing the preset files' Chinese metadata.
      await enPage.keyboard.press('Escape')
      await enPage.getByRole('button', { name: 'Plugins', exact: true }).click()
      await enPage.getByRole('tab', { name: 'Plugin list', exact: true }).click()
      const presetSwitcher = enPage.getByRole('button', { name: 'Choose the agent preset to inspect' })
      await presetSwitcher.waitFor({ timeout: 10_000 })
