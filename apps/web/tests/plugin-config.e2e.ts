...
describe('web e2e: plugin configuration section', () => {
  ...
  /** Revisit Plugins with fresh view state while retaining Host settings. */
  async function openPlugins() {
    ...
    const dialog = page.locator('[data-plugins-page]')
    await dialog.getByRole('tab', { name: '插件配置', exact: true }).click()
    await expect
      .poll(() => page.getByRole('button', { name: '插件', exact: true }).getAttribute('aria-current'), { timeout: 5_000 })
      .toBe('page')
    await expect
